...
public class Notch extends Entity {
    public float rot;
    ...
    static {
        Notch.zombieModel = new ZombieModel();
    }
    
    public Notch(final Level level, final float x, final float y, final float z) {
        super(level);
    ...
    public void render(final float a) {
        GL11.glEnable(3553);
        GL11.glBindTexture(3553, Textures.loadTexture("/notch.png", 9728));
        GL11.glPushMatrix();
        ...
        Notch.zombieModel.render((float)time);
        GL11.glPopMatrix();
        GL11.glDisable(3553);
    }
}
